...
static void checkItem(void) {
	...
	switch (basic_map->map[Grid_y][Grid_x])
	{
		case '.':
			pacman_eatItem(pman,'.');
			//basic_map->map[Grid_y][Grid_x] = NULL;
			break;
		case'P':
	...
	if (is_wall_block(basic_map, Grid_x, Grid_y) || is_room_block(basic_map, Grid_x, Grid_y));
	else {
		if (basic_map->map[Grid_y][Grid_x] == '.') {
			...
			pacman_powerUp();
			for (int i = 0; i < GHOST_NUM; i++) {
				al_set_timer_count(ghosts[i]->flee_timer, 0);
				al_start_timer(ghosts[i]->flee_timer);
...
static void status_update(void) {
	for (int i = 0; i < GHOST_NUM; i++) {
		...
		RecArea pac_area, ghost_area;
		pac_area = getDrawArea(pman->objData, GAME_TICK_CD);
		ghost_area = getDrawArea(ghosts[i]->objData, GAME_TICK_CD);
		if( !cheat_mode && RecAreaOverlap(pac_area, ghost_area))
		{
			if (!pman->powerUp) {
...
static void update(void) {
	...
	if (game_over) {
		...
		*/
		...
		while (al_get_timer_count(pman->death_anim_counter) < 48) {
			draw();
		}
		al_rest(0.8);
		game_log("Game over!");
	...
	int powertime = 0;
	powertime =	al_get_timer_count(power_up_timer);
	
	
	if(pman->powerUp && powertime > 15){
		pman->powerUp = false;
		pacman_stop_music();
		pman->speed = 2;
