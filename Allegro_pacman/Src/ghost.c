...
ALLEGRO_TIMER* ghost_flee_timer;
// [ NOTE - speed again ]
...
Ghost* ghost_create(int flag) {
	...
	ghost->flee_timer = al_create_timer(1.0f);
...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE) {
		int time2 = al_get_timer_count(ghost->flee_timer);
		if (time2 < 12) {
			al_draw_scaled_bitmap(ghost->flee_sprite, 16 * (time % 2), 0,
				16, 16,
			...
			if (time % 2) {
				al_draw_scaled_bitmap(ghost->flee_sprite, 32 + 16 * (time % 2), 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0
				);
		...
		{
		case LEFT:
			al_draw_scaled_bitmap(ghost->move_sprite, 32 + 16*(time % 2), 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
			break;
		...
		case RIGHT:
			al_draw_scaled_bitmap(ghost->move_sprite, 0 + 16*(time % 2), 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
			break;
		case UP:
			al_draw_scaled_bitmap(ghost->move_sprite, 64 + 16*(time % 2), 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
			break;
		...
		case DOWN:
			al_draw_scaled_bitmap(ghost->move_sprite, 96 + 16*(time % 2), 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
			break;
...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
	...
	int side_x = ghost->objData.Coord.x, side_y = ghost->objData.Coord.y;
	...
	if (is_wall_block(M, side_x, side_y) || (room && is_room_block(M, side_x, side_y)))
		return false;
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	Directions fleeDirection = 5 - shortestDirection;
	Directions proba[4], reverse_move, next_move;
	int cnt = 0;
	// Description:
	...
	reverse_move = (5 - ghost->objData.preMove);
	
	for (Directions i = UP; i <= DOWN; i++) {
		if (ghost_movable(ghost, M, i, 1) && i != reverse_move) {
			proba[cnt++] = i;
		}
	}
	
	if (cnt == 0) {
		ghost_NextMove(ghost, reverse_move);
		//game_log("reverse. %d", reverse_move);
		return;
	}
	else {
		if (ghost_movable(ghost, M, fleeDirection, 1) && fleeDirection != reverse_move) {
			ghost_NextMove(ghost, fleeDirection);
			//game_log("flee. %d",fleeDirection);
		}
		else {
			next_move = proba[rand() % cnt];
			ghost_NextMove(ghost, next_move);
			//game_log("next. %d - %d",next_move,cnt);
		}
	}
}
